...
#include "cyapicallbacks.h"
    ...
    ***************************************/
    ...
    const DB_UART_UART_INIT_STRUCT DB_UART_configUart =
    {
        ...
        (uint8) DB_UART_UART_RX_BREAK_WIDTH
    };
    ...
    *******************************************************************************/
    void DB_UART_UartInit(const DB_UART_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            DB_UART_rxBufferSize  =         config->rxBufferSize;

            /* Set TX direction internal variables */
            DB_UART_txBuffer      =         config->txBuffer;
            DB_UART_txDataBits    = (uint8) config->dataBits;
            DB_UART_txBufferSize  =         config->txBufferSize;
            ...
            DB_UART_UART_RX_CTRL_REG = DB_UART_GET_UART_RX_CTRL_MODE(config->stopBits)              |
                                        ...
                                        DB_UART_GET_UART_RX_CTRL_BREAK_WIDTH(config->breakWidth);
...
void DB_UART_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: DB_UART_UartSendBreakBlocking
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void DB_UART_UartSendBreakBlocking(uint32 breakWidth)
    {
        ...
        txIntrReg = DB_UART_GetTxInterruptMode();
        DB_UART_SetTxInterruptMode(0u);
        DB_UART_ClearTxInterruptSource(DB_UART_INTR_TX_UART_DONE);

        /* Store TX CTRL configuration */
        txCtrlReg = DB_UART_TX_CTRL_REG;

        /* Set break width */
        DB_UART_TX_CTRL_REG = (DB_UART_TX_CTRL_REG & (uint32) ~DB_UART_TX_CTRL_DATA_WIDTH_MASK) |
                                        DB_UART_GET_TX_CTRL_DATA_WIDTH(breakWidth);

        /* Generate break */
        DB_UART_TX_FIFO_WR_REG = 0u;

        /* Wait for break completion */
        while (0u == (DB_UART_GetTxInterruptSource() & DB_UART_INTR_TX_UART_DONE))
        {
        }
        ...
        DB_UART_TX_CTRL_REG = txCtrlReg;
        DB_UART_SetTxInterruptMode(txIntrReg);
    }
